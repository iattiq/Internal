...
local function main()
	...
	--local remote = script.Parent.Parent:WaitForChild("Handle")
	
	local OutputLogConfiguration = {
		OutputFormat = {
			...
			[Enum.MessageType.MessageOutput] = {
				title = "Output";
				color = Color3.fromRGB(255, 255 ,255) --Color3.fromRGB(95, 95, 95);
			}
		};
	}
	...
	ScriptViewer.Init = function()
		...
		exe.MouseButton1Click:Connect(function()
			local str = codeFrame:GetText()
			Loadstring(str)()
			--remote:InvokeServer("Exe",codeFrame:GetText())
		end)
		...
		service.LogService.MessageOut:Connect(function(message, messageType)
			addOutput({
				Text = message,
				Time = tick(),
				Color = OutputLogConfiguration.OutputFormat[messageType].color
			})
		end)
		...
		local function getOutput()
			...
			local LogHistory = service.LogService:GetLogHistory()
			
			local allOutput = {}
				
			for i, data in LogHistory do
				allOutput[i] = {
					Text = data.message,
					Time = data.timestamp,
					Color = OutputLogConfiguration.OutputFormat[data.messageType].color
				}
			end
		...
		codeFrame:SetText('print("Hello World!")')
	end
