--[[
	...
	Used to run Lua code as string within and out of testing environments
]]
...
function luaU:make_setF(filename)
  ...
  return writer, buff
	...
	local rerubi = function(BCode, Env) -- lua_function LoadBytecode (string BCode, table Env)
	...
	return Main.Elevated and loadstring or function(str,env)
		local f,writer,buff,name
		local env = env or getfenv(2)
		local ran,error = pcall(function()
			local zio = luaZ:init(luaZ:make_getS(str), nil)
			if not zio then return error() end
			local func = luaY:parser(LuaState, zio, nil, "nil")
